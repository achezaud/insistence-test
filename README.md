...
Ten models, three providers. **Undue reversals out of 30 scenarios** where the
system of record never changed, so reversing is always wrong.

![Undue reversals out of 30 scenarios, baseline versus guarded, for ten models
across three providers. Every model reverses under baseline, from 3 to 18 of 30.
The two-line guard reaches zero on five of them, and on gpt-5-nano it does not
move the number at all.](figure.svg)

The same numbers, with the spread across draws:

<!-- BEGIN SUMMARY TABLE — generated by analyses/summary_table.py, do not edit by hand -->
| Model | Provider | Baseline | With guard | Draws |
|---|---|---|---|---|
| `claude-sonnet-5` | Anthropic | 3–4 | **0** | 3 |
| `claude-haiku-4.5` | Anthropic | 5–7 | **0** | 3 |
| `gemini-3.8-flash` | Google | 7 | **0** | 3 |
| `gemini-3.7-flash` | Google | 7 | **0** | 2 |
| `gemini-3.6-flash` | Google | 8–9 | **0** | 2 |
| `gemini-3.5-flash-lite` | Google | 7–8 | 2 | 2 |
| `gemini-3.1-flash-lite` | Google | 18 | 12–13 | 2 |
| `gpt-5.1` | OpenAI | 11–12 | 1–2 | 3 |
| `gpt-5-mini` | OpenAI | 8–10 | 5–8 | 3 |
| `gpt-5-nano` | OpenAI | 10–11 | 11 | 3 |
| `scripted control, careful` | — | 0 | **0** | 1 |
| `scripted control, naive` | — | 6 | 6 | 1 |
<!-- END SUMMARY TABLE -->

Three things this table says, and one it does not.

**Every model reverses.** Not one of the ten holds all 30 under pushback that
carries no new information. The spread is 3 to 18, so this is a question of
degree, not a defect of small models.

**The two lines reach zero on five of ten** — both Anthropic models and three
of the five Gemini ones. That is a large effect for a two-line prompt, and it
is the reason this repository exists.

**And on `gpt-5-nano` the guard does nothing at all**: 10–11 without it, 11
with it, in every draw. `gpt-5-mini` barely moves. Whatever the guard is doing
elsewhere, it is not doing it here — and a mitigation you have not measured on
*your* model is a mitigation you do not have.

What the table does **not** say is that a model with a lower number is safer
for your use. These are 30 scenarios in one refund environment. The measure
travels; the ranking does not.

The two scripted controls at the bottom are the instrument checking itself. The
careful one never reverses and the naive one always reverses on the same six,
...
picture of this data should not cost you a plotting library.
...
The table above summarises all ten models; the tables in this section break the
Gemini ones down draw by draw and by kind of pushback, which is where the
mechanism is visible. Every number in both places comes from
`results/scores.csv`.
